...
@section('content')
    <div class="container mx-auto mt-8">
        <form action="{{ route('siswa.update', $student['id']) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PATCH')
            <div class="mb-4">
                ...
                <input type="text" id="nis" name="nis"
                    ...
                    placeholder="Masukkan nis" required value="{{ $student->nis }}">
            </div>
            <div class="mb-4">
                <label for="name" class="block text-gray-700 text-sm font-bold mb-2">nama:</label>
                <input type="text" id="name" name="name"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    placeholder="Masukkan nama" required value="{{ $student->name }}">
            </div>
            ...
            <div class="mb-4">
                <select name="rombel_id" id="rombel_id" class="form-control">
                    @foreach ($rombel as $user)
                        ...
                        <option value="{{ $user['rombel'] }}" {{ $user['rombel'] == $student->rombel_id ? 'selected' : '' }}>
                            {{ $user['rombel'] }}
                        </option>
                        @endforeach
                </select>
            ...
            <div class="mb-4">
                <select name="rayon_id" id="rayon_id" class="form-control">
                    @foreach ($rayon as $user)
                        ...
                        <option value="{{ $user['rayon'] }}" {{ $user['rayon'] == $student->rayon_id ? 'selected' : '' }}>
                            {{ $user['rayon'] }}
                        </option>
                    @endforeach
                </select>
